    ...
    class BeRocket_Framework {
        ...
        public function plugins_loaded() {
            if( ! empty($_POST[ $this->cc->values[ 'settings_name' ] ]) ) {
                $this->post = $_POST[ $this->cc->values[ 'settings_name' ] ];
            }
        }
        ...
        public function get_product_data_berocket() {
            ...
            if( true && $curl = curl_init() ) {
                curl_setopt($curl, CURLOPT_URL, 'http://berocket.com/main/get_product_data/'.$this->info['id']);
        ...
        public function plugin_action_links( $links ) {
            $action_links = array(
                'settings' => '<a href="' . admin_url( 'admin.php?page=' . $this->cc->values[ 'option_page' ] ) .
                              '" title="' . __( 'View Plugin Settings', $this->cc->info[ 'domain' ] ) . '">' .
                              __( 'Settings', $this->cc->info[ 'domain' ] ) . '</a>',
            );
            ...
            if ( $file == $plugin_base_slug ) {
                $row_meta = array(
                    'docs'    => '<a href="http://berocket.com/docs/plugin/' .
                                 ...
                                 __( 'View Plugin Documentation', $this->cc->info[ 'domain' ] ) .
                                 '" target="_blank">' . __( 'Docs', $this->cc->info[ 'domain' ] ) . '</a>',
                    'premium' => '<a href="http://berocket.com/product/' . $this->cc->values[ 'premium_slug' ] .
                                 '" title="' . __( 'View Premium Version Page', $this->cc->info[ 'domain' ] ) .
                                 '" target="_blank">' . __( 'Premium Version', $this->cc->info[ 'domain' ] ) . '</a>',
                );

                return array_merge( $links, $row_meta );
            }

            return apply_filters('brfr_plugin_row_meta_' . $this->cc->info[ 'plugin_name' ], (array) $links, $file);
        }
        ...
        public function admin_menu() {
            if ( method_exists( 'BeRocket_updater', 'get_plugin_count' ) && BeRocket_updater::get_plugin_count() > 3 ) {
                add_submenu_page(
                    'berocket_account',
                    __( $this->cc->info[ 'norm_name' ] . ' settings', $this->cc->info[ 'domain' ] ),
                    __( $this->cc->info[ 'norm_name' ], $this->cc->info[ 'domain' ] ),
                    'manage_options',
        ...
        public function admin_settings( $tabs_info = array(), $data = array() ) {
            $this->display_admin_settings($tabs_info, $data);
        }
        ...
        public function display_admin_settings( $tabs_info = array(), $data = array(), $setup_style = array() ) {
            ...
            $def_setup_style = array(
                ...
                'header_name' => "{$this->cc->info['full_name']} by BeRocket",
            ...
            if ( isset($data) and is_array($data) and count( $data ) ) {
                ...
                foreach ( $tabs_info as $tab_name => $tab_info ) {
                    ...
                    $page_menu .= '<a href="' . ( empty($tab_info['link']) ? '#berocket_framework_menu_' . sanitize_title( $tab_name ) : $tab_info['link'] ) . '" class="' . ( empty($tab_info['priority']) ? 'default' : $tab_info['priority'] ) . ( empty($tab_info['link']) ? '' : ' redirect_link' ) . ( ( $is_first ) ? ' active' : ''  ) .
                    ...
                    $page_menu .= __( $tab_name, $this->cc->info['domain'] ) . '</a>';
                    ...
                    if ( $is_first ) {
                        ...
                        $title .= __( $tab_name, $this->cc->info['domain'] );
                    }
                ...
                foreach ( $data as $tab_name => $tab_content ) {
                    $page_content .= '<div class="nav-block berocket_framework_menu_' . sanitize_title( $tab_name ) . '-block ' . ( ( $is_first ) ? 'nav-block-active' : ''  ) . '">';
                    ...
                    if ( isset($tab_content) and is_array($tab_content) and count( $tab_content ) ) {
                        foreach ( $tab_content as $item ) {
                            ...
                            if ( empty($item['section']) or $item['section'] == 'field' ) {
                                $page_content .= '<th scope="row">' . __( $item['label'], $this->cc->info['domain'] ) .'</th><td>';
                                ...
                                foreach( $field_items as $item_key => $field_item ) {
                                    ...
                                    if ( empty($field_item['type']) or $field_item['type'] == 'text' ) {
                                        $item_content .= '<label><input type="text" name="' . $field_name
                                             . '" value="' . $value . '"' . $class . $extra . '/>' . $field_item['label_for'] . '</label>';
                                    } elseif ( $field_item['type'] == 'number' ) {
                                        $item_content .= '<label><input type="number" name="' . $field_name
                                             . '" value="' . $value . '"' . $class . $extra
                                             . ( empty($field_item['min']) ? '' : ' min="' . $field_item['min'] . '"' ) . ( empty($field_item['max']) ? '' : ' max="' . $field_item['max'] . '"' ) . '/>' . $field_item['label_for'] . '</label>';
                                        ...
                                        $item_content .= '<label><input type="radio" name="' . $field_name
                                             . '" value="' . $field_item['value'] . '"'
                                             . ( $field_item['value'] == $radio_default ? ' checked="checked" ' : '' )
                                             . $class . $extra . '/>' . $field_item['label_for'] . '</label>';
                                    } elseif ( $field_item['type'] == 'checkbox' ) {
                                        $item_content .= '<label><input type="checkbox" name="' . $field_name
                                             . '" value="' . $field_item['value'] . '"' .
                                             ( ( ! empty($option_values) ) ? ' checked="checked" ' : '' ) . $class . $extra . '/>' . $field_item['label_for'] . '</label>';
                                    } elseif ( $field_item['type'] == 'selectbox' ) {
                                        $item_content = '<label><select name="' . $field_name
                                             . '"' . $class . $extra . '>';
                                        if ( isset($field_item['options']) and is_array($field_item['options']) and count( $field_item['options'] ) ) {
                                            foreach ( $field_item['options'] as $option ) {
                                                $item_content .= '<option value="' . $option['value'] . '"' .
                                                     ( ( $value == $option['value'] ) ? ' selected="selected" ' : '' )
                                                     . '>' . $option['text'] . '</option>';
                                            }
                                        } else {
                                            $item_content .= "<option>Options data is corrupted!</option>";
                                        }

                                        $item_content .= '</select>' . $field_item['label_for'] . '</label>';
                                    } elseif ( $field_item['type'] == 'textarea' ) {
                                        $item_content .= '<textarea name="' . $field_name
                                             . '"' . $class . $extra . '>'. $value . '</textarea>';
                                    } elseif ( $field_item['type'] == 'color' ) {
                                        $item_content .= br_color_picker( $field_name, $value, ( isset($field_item['value']) ? $field_item['value'] : '' ), $field_item);
                                    } elseif ( $field_item['type'] == 'image' ) {
                                        $item_content .= br_upload_image( $field_name, $value, $field_item);
                                    } elseif ( $field_item['type'] == 'faimage' ) {
                                        $item_content .= br_fontawesome_image( $field_name, $value, $field_item);
                                    } elseif ( $field_item['type'] == 'fontawesome' || $field_item['type'] == 'fa' ) {
                                        $item_content .= br_select_fontawesome( $field_name, $value, $field_item);
                                    } elseif ( $field_item['type'] == 'products' ) {
                                        $item_content .= br_products_selector( $field_name, $value, $field_item);
                                    } else {
                                ...
                                $page_content .=  '
                                    <th scope="row">' . __('Plugin key', $this->cc->info['domain']) .'</th>
                                    <td><div class="license-block">
                                        ...
                                        <br />
                                        <span style="color:#666666;margin-left:2px;">' . __('Key for plugin from BeRocket.com', $this->cc->info['domain']) . '</span>
                                        <br />
                ...
                echo '<div class="content">';
                    ...
                    if( ! $setup_style['hide_form'] ) {
                        ...
                        if( ! $setup_style['hide_save_button'] ) {
                            echo '<input type="submit" class="button-primary button" value="' . __( 'Save Changes', $this->cc->info['domain'] ) . '" />';
        ...
        public function save_settings() {
            if ( current_user_can( 'manage_options' ) ) {
                if ( isset( $_POST[ $this->cc->values[ 'settings_name' ] ] ) ) {
                    update_option(
                        $this->cc->values[ 'settings_name' ],
                        self::sanitize_option( $_POST[ $this->cc->values[ 'settings_name' ] ] )
                    );

                    echo json_encode( $_POST[ $this->cc->values[ 'settings_name' ] ] );
                }
            }
