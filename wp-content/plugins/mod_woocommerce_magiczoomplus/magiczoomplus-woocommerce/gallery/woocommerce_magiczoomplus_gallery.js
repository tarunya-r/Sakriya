(function($) {
    ...
    SlideInstance.prototype = $.extend(SlideInstance.prototype, {
        ...
        loadSelector: function (node) {
            ...
            if (!this.options.type && !this.selectorState) {
                ...
                if (img) {
                    ...
                    self.thumbUrl = $(img).attr('src');
                } else {
                    this.selectorState = 1;
                    this.tool.MagicToolboxGalleryGetVideoImage(function(url) {
        ...
        setSelectorImage: function (node) {
            if (this.thumbUrl && !$(node).find('img').length) {
                $(node).append($('<img>').attr('src', this.thumbUrl));
            }
        },
    ...
    $.fn.MagicToolboxGalleryGetVideoImage = function (callback) {
        var sources = {};
