...
jQuery(document).ready(function ($) {
	...
	$(".install").on('click', function (e) {
    	...
    	var ajaxurl = storevillaWelcomeObject.ajaxurl;

		$.ajax({
			type: 'POST',
			url: ajaxurl,
			data: {
				action: 'storevilla_plugin_installer',
				plugin: plugin,
				plugin_file: plugin_file,
				nonce: storevillaWelcomeObject.admin_nonce,
			},
			success: function(response) {
		   		...
		   		if(response == 'success'){
				   		...
				   		el.html(storevillaWelcomeObject.installed_btn);
	...
	$('.install-offline').on('click', function (e) {
		...
		$.ajax({
			...
			url: ajaxurl,
			data: {
				action: 'storevilla_plugin_offline_installer',
	...
	$(".activate").on('click', function (e) {
		...
		$.ajax({
	   		...
	   		url: ajaxurl,
	   		data: {
	   			action: 'storevilla_plugin_activation',
	   			plugin: plugin,
	   			nonce: storevillaWelcomeObject.activate_nonce,
	   			dataType: 'json'
	   		},
	   		success: function(response) {
		   		if(response){
	...
	$('.activate-offline').on('click', function (e) {
		...
		$.ajax({
	   		...
	   		url: ajaxurl,
	   		data: {
	   			action: 'storevilla_plugin_offline_activation',
	   			plugin: plugin,
	   			nonce: storevillaWelcomeObject.activate_nonce,
	   			dataType: 'json'
	   		},
