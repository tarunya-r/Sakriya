		...
		foreach ( $options as $value ) {
			...
			case "images":
				...
				foreach ( $value['options'] as $key => $option ) {
					...
					$output .= '<img src="' . esc_url( $option ) . '" alt="' . $option .'" class="of-radio-img-img' . $selected .' '. esc_attr( $key ) .'" onclick="document.getElementById(\''. esc_attr($value['id'] .'_'. $key) .'\').checked=true;" />';
				}
				break;
